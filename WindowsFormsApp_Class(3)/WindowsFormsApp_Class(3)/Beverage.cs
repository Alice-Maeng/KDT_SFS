...
namespace WindowsFormsApp_Class_3_
{
    ...
    {
        // #1. 오버로드 - 같은 이름의 메서드를 매개변수만 다르게 해서 여러 개 정의 하는 것!
        ...
        // #2. 오버라이드 - 부모 클래스의 메서드를 자식 클래스에서 다시 재정의(덮어쓰기) 하는 것!
        // why? 부모 클래스가 기본 기능을 제공하되, 자식 클래스에서는 그 기능을 자신만의 방식으로 바꿔야할 때가 많을때 사용
        // 조건)
        // 1. 부모 메서드가 virtual 키워드로 선언되어 있어야 하고,
        // 2. 자식 메서드에서 override 키워드로 재정의 해야 함.
    ...
    {
        /* 하기 코드는 오류 발생 --- Latte 클래스가 sealed 되어 있기땜에 오류남
        public override void Prepare()
        {
    ...
    {
        // Latte클래스를 상속 받지 않았기 떄문에 prepare() 메서드 사용 가능 
        // 즉 sealed 영향을 안받기 때문에 override 가능 
        public override void Prepare()
        {
            Console.WriteLine("콜드브루를 준비하고 있습니다.");
        }
